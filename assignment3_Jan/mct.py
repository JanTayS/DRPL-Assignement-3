...
class MonteCarloTree:
    ...
    def uct(self, node):
        ...
        for child in self.children[node]:
            uct = self.q[child] / self.n[child] + self.c * sqrt(log(self.n[node]) / self.n[child])
            best = child
            max = uct
            break
        for child in self.children[node]:
    ...
    def choose(self, node):
        ...
        for child in self.children[node]:
            ...
            child_position = 0
            position = 0
            for pos1, pos2 in zip(child.game_state,node.game_state):
                if pos1 != ' ' and pos1 != pos2:
                    child_position = position
                position += 1
            scores.append((child_position,score))
            # print(child_position,self.q[child],self.n[child])

        choose_matrix = np.zeros(9)
        for score in scores:
            choose_matrix[score[0]] = score[1]

        # choose_matrix = choose_matrix.reshape((3,3))
        # plt.matshow(choose_matrix)
        # plt.colorbar()
        # plt.show()
